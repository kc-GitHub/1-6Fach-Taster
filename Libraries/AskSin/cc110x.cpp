...
void cc110x::init(void) {															// initialize CC1101
	#if defined(ENABLE_CC110X_DEBUG)											// some debug message
...
boolean cc110x::sendData(uint8_t *buf, uint8_t burst) {								// send data packet via RF
...
//	trx868.rfState = RFSTATE_RX;												// declare to be in Rx state
	...
	#if defined(ENABLE_CC110X_DEBUG)											// debug message, string should be short, otherwise program stops
...
void cc110x::setPowerDownxtStatte() {													// put CC1101 into power-down state
